...
1. User clicks "Capture screenshot" in the popup (the popup then closes — also when the page has shown the setup-required modal, so the user can read it) or right-clicks → "Add screenshot to Google Calendar" (on any page, but **not while text is highlighted**: the item is registered for `["page", "frame", "link", "image", "video", "audio"]`, so a Selection leaves only the Selection item on the menu. `"editable"` is out of that list too, because a selection inside a field carries both contexts and would otherwise put the Screenshot item back exactly where it is meant to be absent — the cost is that a right-click in an empty field offers no Screenshot, and the popup button covers that. Hidden entirely by the popup toggle stored as `showScreenshotMenuItem` in sync storage, default on — the service worker re-applies its menus on change, item by item with `contextMenus.update`/`create` rather than clearing the menu, so the Selection item is never off it)
...
CI runs them: `.github/workflows/test-extension.yml` has a `backend` job alongside the Playwright one — a sibling rather than a step, so a red Deno suite and a red Playwright suite are told apart at a glance. A failing backend test fails the pull request.

### Test Organization
- `tests/*.test.js`: Test suites — `configuration`, `llm-prompt`, `llm-prompt-sync`, `selection-extraction`, `screenshot-pipeline`, `region-overlay`, `screenshot-extraction`, `screenshot-key-path`, `context-menu`, `usage-refresh`, `eval-screenshot-render`
...
- `stubBackend` fixture: starts/stops the server; `stub.events` is the canned answer for `process-text`, `process-image` and the OpenAI route alike, `stub.usage` for the two Edge Functions and for the read-only `get-usage` the popup asks on open (the OpenAI route returns events only), `stub.requestsTo(pathname, method)` the assertions. `stub.textResponse` / `stub.imageResponse` / `stub.openAiResponse` / `stub.usageResponse` (`{ status, body }`) make one endpoint fail instead; `stub.openAiContent` hands the own-key path raw model output (fenced JSON, a single event object, empty) so normalisation can be driven; `stub.responseDelayMs` holds the Edge Function answers back so a test can act mid-Extraction
- `stubbedEndpoints` fixture: writes the stub's base URL to `backend_base_url_override` in `chrome.storage.local`. `signedIn` builds on it by letting the worker's own start-up settle, tearing that client down (it is aimed at production, and a client nobody holds still answers auth events — its `SIGNED_OUT` would clear the seeded session), then seeding a session in `supabase_session`, resetting the memoised start-up (`authStartUp`/`supabaseAuth`) and re-running `initializeAuth()` — production deliberately never rebuilds its single client; `ownKey` builds on it with an OpenAI key in `chrome.storage.sync`, for the own-key path
