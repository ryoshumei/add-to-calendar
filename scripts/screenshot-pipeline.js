...
const SCREENSHOT_PIPELINE = {
    ...
    MAX_EDGE_PX: 1600,
    JPEG_QUALITY: 0.7,
    // A Screenshot this large never survives the downscale above, so passing
    // it on would mean something went wrong; the Extraction is refused instead.
    MAX_ENCODED_BYTES: 10 * 1024 * 1024,

    /**
     * Build the Screenshot to send for Extraction: the Region of the capture,
     * downscaled so its longest side is at most MAX_EDGE_PX, encoded as JPEG.
     *
...
// Retina display the Region has to be scaled up to land on the same content.
// No Region means the whole visible tab.
